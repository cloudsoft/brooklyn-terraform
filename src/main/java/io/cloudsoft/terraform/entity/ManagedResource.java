...
public interface ManagedResource extends BasicEntity, Startable {
    ...
    @SetFromFlag("type")
    ConfigKey<String> TYPE =  ConfigKeys.newStringConfigKey("tf.resource.type", "Terraform resource type (vsphere_virtual_machine, aws_instance, vpc, security group, etc)", null);

    @SetFromFlag("provider")
    ConfigKey<String> PROVIDER =  ConfigKeys.newStringConfigKey("tf.resource.provider", "Terraform resource provider (cloud provider)", null);
